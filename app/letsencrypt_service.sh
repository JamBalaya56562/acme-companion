...
function update_cert {
    ...
    local -n legacy_accountemail_ref="LETSENCRYPT_${cid}_EMAIL"
    local -n accountemail_ref="ACME_${cid}_EMAIL"
    accountemail="${accountemail_ref:-${legacy_accountemail_ref}}"
    local config_home
        ...
        if parse_true "${eab_registration}"; then
            if [[ -n "${accountemail// }" ]]; then
                ...
                echo "Warning: registering a new ACME account with EAB but no contact email." \
                    ...
                    "Set ACME_EMAIL or DEFAULT_EMAIL to provide it."
            fi
        fi
    fi
